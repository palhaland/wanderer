...
type bulkMergeRequest struct {
	SourceTrailIDs       []string                 `json:"sourceTrailIds"`
	TargetTrailID        string                   `json:"targetTrailId"`
	Settings             trailmerge.MergeSettings `json:"settings"`
	GeneratePerfectTrack bool                     `json:"generatePerfectTrack"`
}

func TrailMergeBulk(client meilisearch.ServiceManager) func(e *core.RequestEvent) error {
	return func(e *core.RequestEvent) error {
		if e.Auth == nil {
			return apis.NewUnauthorizedError("trail_merge_auth_required", nil)
		}
		...
		for _, srcID := range request.SourceTrailIDs {
			source, err := e.App.FindRecordById("trails", srcID)
			if err != nil {
				return apis.NewBadRequestError("trail_merge_source_not_found", err)
			}
			if !trailmerge.CanMerge(e.App, userActor.Id, source, target, request.Settings.Delete) {
		...
		newTrailID, err := trailmerge.BulkMerge(e.App, client, ctx, userActor, request.SourceTrailIDs, request.TargetTrailID, request.Settings, request.GeneratePerfectTrack)
		if err != nil {
			return apis.NewBadRequestError(err.Error(), err)
		}
